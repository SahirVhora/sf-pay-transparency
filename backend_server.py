...
class PayTransparencyHandler(BaseHTTPRequestHandler):
    ...
    def _api_evidence_pack(self, payload: dict):
        ...
        article9 = _calculate_article9(
            {"_raw": raw_evidence}, resolved, suppression_min_group
        )
        self._json(
            200,
            {
...
def _calculate_article9(
    evidence: dict, resolved: dict[str, str | None], suppression_min_group: int = 10
) -> dict:
    ...
    for (country, category, currency), rows in sorted(groups.items()):
        ...
        if (
            employees < suppression_min_group
            or len(female) < suppression_min_group
            or len(male) < suppression_min_group
        ):
            ...
            category_results.append(
                {
                    ...
                    "medianMalePay": None,
                    "medianGapPct": None,
                    "variableComponentRecords": sum(
                        1 for row in rows if row["isVariable"]
                    ),
...
def _article9_limitations(
    ...
    notes.append(
        "Small-population category results are suppressed before displaying or exporting Article 9 pay metrics."
    )
    return notes
